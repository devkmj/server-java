...
public class OrderServiceTest {
    ...
    @Test
    @DisplayName("정상적인 주문 요청일 경우 주문이 성공한다")
    void 정상_주문_성공(){
        // given
        Long userId = 1L;
        Long productId = 1L;
        ...
        Product product = new Product("맥북", price, ProductStatus.AVAILABLE);
        ProductStock stock = new ProductStock(product, 5); // 충분한 재고
        ...
        // when & then
        assertThatCode(() -> orderService.order(userId, productId, qty, null))
    ...
    void 재고_부족_시_예외가_발생한다(){
        ...
        // when & then
        assertThatThrownBy(() -> orderService.order(userId, productId, qty, null))
    ...
    void 쿠폰이_유효하지_않을_경우_예외가_발생한다(){
        ...
        assertThatThrownBy(() -> orderService.order(userId, productId, qty,  userCouponId))
                .isInstanceOf(IllegalArgumentException.class)
                .hasMessageContaining("유효하지 않은 쿠폰입니다");
    }

    @Test
    @DisplayName("상품이 유효하지 않을 경우 예외가 발생한다")
    void 상품이_유효하지_않을_경우_예외가_발생한다(){
        //given
        Long userId = 1L;
        Long productId = 1L;
        int qty = 1;
        ...
        //상품 상태가 판매중이 아님
        Product invalidProduct = new Product("에어팟", 320000, ProductStatus.AVAILABLE);
        ProductStock stock = new ProductStock(invalidProduct, 5);
        Balance balance = new Balance(userId, 5000000);

        given(productRepository.findById(productId)).willReturn(Optional.of(invalidProduct));
        given(productStockRepository.findByProductId(productId)).willReturn(Optional.of(stock));
        given(balanceRepository.findByUserId(userId)).willReturn(Optional.of(balance));
        ...
        assertThatThrownBy(() -> orderService.order(userId, productId, qty, null))
                .isInstanceOf(IllegalArgumentException.class)
                .hasMessageContaining("유효하지 않은 상품입니다.");
    ...
    void 존재하지_않는_상품_ID일_경우_예외() {
        ...
        assertThatThrownBy(() -> orderService.order(userId, productId, 1, null))
                .isInstanceOf(IllegalArgumentException.class)
